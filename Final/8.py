...
def iteration(matrix, beta, field):
  ...
  chai = np.mean(maglistchai) - magnetisation ** 2
  chi = beta * np.var(spinlist) / size**2
  print ('Run over.')

  return energy, capacity, magnetisation, chi, matrix, energydcub, chai
...
def avgvalues(beta, field, size, runs):
  ...
  for i in range(runs):
    ...
    energy, capacity, mag, chi, resultmatrix , edcub, chai= iteration(matrix, beta, field)
    Elist.append(energy)
...
while T[-1] <= 5:
    T.append(T[-1]+0.1)
    beta=1/T[-1]    
    Ch_N.append(avgvalues(beta, 0, 10, 3)/T[-1]**2)
pl.plot(T, Ch_N, '-',color = 'b')

pl.title('Ising model Monte Carlo($10^2$ Lattice)')
pl.xlabel('Temperature')
pl.ylabel('Variance of M')
pl.xlim(0,5.5)
