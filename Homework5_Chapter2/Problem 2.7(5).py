...
class flying_cannon:
    ...
    def calculate_c(self):
        while not self.yno[-1] < 0 :
            vxno = self.v_xno[-1] - self.v_xno[-1] * self.vno[-1] * self.constno[0] * self.dt
            vyno = self.v_yno[-1] - self.v_yno[-1] * self.vno[-1] * self.constno[0] * self.dt - self.g * self.dt         
    ...
    def show_results(self):
        pl.plot(self.x, self.y, 'g', label='The trajectory of the cannon using the adiabatic model')
        pl.plot(self.xno, self.yno, 'b', label='The trajectory of the cannon without density correction')
        pl.xlim(0.0, 30000.0)
        pl.ylim(0.0, 10000.0)
        pl.xlabel('x ($m$)')
        pl.ylabel('y ($m$)')
