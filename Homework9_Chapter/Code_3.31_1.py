...
class the_billiaed_problem :
    ...
    def calculation(self):
        while self.t[-1] <= self.T:
            ...
            if   self.x[-1]**2/4+self.y[-1]**2/3 > 1.0:
                 self.x[-1],self.y[-1] = self.approximation('x**2/4+y**2/3 < 1',self.x[-2], self.y[-2], self.vx[-2], self.vy[-2])
                 self.vx[-1],self.vy[-1] = self.reflection((3/4) *self.x[-1], self.y[-1], self.vx[-2], self.vy[-2])
    ...
    def plot_boundary(self):
        ...
        y = []
        while theta < np.pi:
            x.append(2*np.cos(theta))
            y.append((3**0.5)*np.sin(theta))
            theta+= 0.01
        pl.plot(x,y,'g.')
...
a = the_billiaed_problem(2, 0, 1,0.5 )
a.calculation()
a.plot1()
a.plot_boundary()
pl.show()
